...
import { triggerUserBanned } from './bannedSlice';
import { showSnack } from './snackbarSlice';
...
// Reader pattern (RP3): Subscription expired snackbar throttle.
// Birden fazla query aynı anda 403 alabilir → tekrar tekrar snackbar gösterilmesin.
let subscriptionExpiredSnackThrottle = 0;
// Ban snackbar throttle — aynı kullanıcı için birden fazla gösterilmesin.
let bannedSnackThrottle = 0;
...
/**
 * Sunucu refresh rotation + "reuse" kuralı: aynı düz refresh iki kez hızlıca kullanılırsa
 * tüm token ailesi iptal olabiliyor. Çoklu hesapta `refreshInactiveAccounts`, `pruneStale`
 * ve RTK reauth aynı anda tetiklenirse yarış oluşur — tüm Auth/refresh POST'larını tek sıraya al.
 */
let refreshHttpChain: Promise<unknown> = Promise.resolve();

function runSerializedRefreshHttp<T>(fn: () => Promise<T>): Promise<T> {
  const run = refreshHttpChain.then(fn, fn);
  refreshHttpChain = run.then(
    () => undefined,
    () => undefined
  );
  return run;
...
> {
  return runSerializedRefreshHttp(async (): Promise<
    | { ok: true; accessToken: string; refreshToken: string }
    | { ok: false; reason: TokenRefreshFailureReason }
  > => {
...
export const baseQueryWithReauth: BaseQueryFn<any, unknown, FetchBaseQueryError> =
  async (args, api, extra) => {
    try {
      ...
      // Kullanıcı banı: backend 403 + banned:true döner (UserStatusFilter).
      // Token refresh denemesi yapma — diğer hesaba geç veya giriş sayfasına yönlendir.
      if (res.error?.status === 403) {
        const errData = res.error.data as any;
        const banMsg: string = errData?.message || errData?.Message || '';
        if (errData?.banned === true || banMsg.includes('askıya alınmıştır')) {
          const now = Date.now();
          if (!bannedSnackThrottle || (now - bannedSnackThrottle) > 5000) {
            bannedSnackThrottle = now;
            const displayMsg = banMsg || 'Hesabınız yönetici tarafından askıya alınmıştır.';
            (api as any).dispatch(showSnack({ message: displayMsg, isError: true }));
          }
          (api as any).dispatch(triggerUserBanned());
          return res;
        }
      }
      ...
      // Trial konsepti kaldırıldı (Madde 8/Phase B); backend artık sadece
      // Messages.SubscriptionExpired ("Aboneliğiniz sona ermiştir...") döner.
      // Reader pattern (RP3): Sessiz yönlendirme yerine kullanıcıya da snackbar gösterilir.
      if (res.error?.status === 403) {
        const errData = res.error.data as any;
        const msg: string = errData?.message || errData?.Message || '';
        if (msg.includes('Aboneliğiniz') || msg.includes('abone')) {
          (api as any).dispatch(triggerSubscriptionExpired());
          // Snackbar — yönlendirme öncesi kullanıcıya neden olduğunu söyle.
          // Çoklu 403 durumunda aynı snackbar tekrar tekrar gösterilmesin diye basit throttle.
          const now = Date.now();
          if (!subscriptionExpiredSnackThrottle || (now - subscriptionExpiredSnackThrottle) > 4000) {
            subscriptionExpiredSnackThrottle = now;
            try {
              const message = i18n.t('errors.subscriptionExpired') as string;
              (api as any).dispatch(showSnack({ message, isError: true }));
            } catch {
              // i18n init edilmemiş olabilir (early bootstrap) — sessizce geç.
            }
          }
          return res;
        }
      ...
      if ((res.error?.status === 401 || res.error?.status === 403 || res.error?.status === 419 || res.error?.status === 498) && tokenStore.refresh) {
        if (!refreshing) {
          refreshing = (async () => {
            ...
            try {
              const r = await runSerializedRefreshHttp(() =>
                Promise.resolve(
                  rawNoAuth(
